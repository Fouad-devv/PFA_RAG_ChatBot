    ...
    <>
      ...
      <button
        ...
        className={`hidden lg:flex fixed top-1/2 -translate-y-1/2 z-[60]
          ...
          w-5 h-10 bg-[#1a2234] border border-white/[0.1] border-l-0
          rounded-r-xl shadow-md
          hover:bg-emerald-500/10 hover:border-emerald-500/30 hover:shadow-lg
          transition-all duration-300 ease-in-out
          ${sidebarCollapsed ? "left-0" : "left-72"}`}
      >
        <svg
          className="w-3 h-3 text-slate-400 group-hover:text-emerald-400"
      ...
      >
        ...
        <div className="px-3 py-3 border-b border-white/[0.07]">
          <div className="relative">
            ...
            <input
              className="w-full pl-8 pr-7 py-2 text-sm bg-white/[0.05] border border-white/[0.1] rounded-xl placeholder-slate-500 text-slate-200 focus:outline-none focus:ring-2 focus:ring-emerald-500/40 focus:border-emerald-500/40 transition"
            ...
            <div className="text-center py-16 px-3">
              {searchQuery ? (
                <>
                  ...
                  <p className="text-sm font-semibold text-slate-300">Aucun résultat</p>
                  <p className="text-xs text-slate-500 mt-1">pour « {searchQuery} »</p>
                </>
              ) : (
                <>
                  <div className="w-12 h-12 rounded-2xl bg-white/[0.06] flex items-center justify-center mx-auto mb-3">
                  ...
                  <p className="text-xs text-slate-500 mt-1">Démarrez un nouveau chat !</p>
                </>
              )}
            </div>
          ) : (
        ...
        {openSpaceUser && (
          <div className="mx-2 mb-2 rounded-2xl overflow-hidden border border-white/[0.1] animate-scale-in shadow-xl shadow-black/30">
            <div className="bg-gradient-to-br from-[#1a2234] to-[#0f172a] p-4">
        ...
        <div className="px-2 py-3 border-t border-white/[0.07]">
          <button
            ...
            className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all ${
              openSpaceUser ? "bg-white/[0.1]" : "hover:bg-white/[0.06]"
            }`}
          >
            <div className="w-9 h-9 rounded-full bg-gradient-to-br from-emerald-400 to-emerald-600 flex items-center justify-center text-white font-bold text-sm flex-shrink-0 shadow-md shadow-emerald-900/50">
              {username?.charAt(0).toUpperCase()}
            </div>
            ...
            <svg className={`w-4 h-4 text-slate-500 transition-transform duration-200 flex-shrink-0 ${openSpaceUser ? "rotate-180" : ""}`}
...
const ConvItem = ({
  ...
  <div
    ...
    className={`relative px-3 py-2.5 rounded-xl cursor-pointer transition-all group border ${
      isActive
        ? "bg-emerald-500/[0.08] border-emerald-500/35 shadow-sm shadow-emerald-500/10"
        : "border-white/[0.08] hover:bg-white/[0.05] hover:border-white/[0.14]"
    }`}
  >
    {isActive && (
      <span className="absolute left-0 top-2 bottom-2 w-[3px] bg-gradient-to-b from-emerald-400 to-emerald-600 rounded-full" />
    )}
    <div className="flex items-center gap-2.5 pr-6 min-w-0 pl-1">
      <svg
        className={`w-3.5 h-3.5 flex-shrink-0 transition-colors ${isActive ? "text-emerald-400" : "text-slate-600 group-hover:text-slate-400"}`}
        fill="currentColor" viewBox="0 0 24 24"
      >
      ...
      {isRenaming ? (
        <input
          ref={renameInputRef}
          className="flex-1 text-sm bg-[#1a2234] border border-emerald-400 rounded-lg px-2 py-0.5 text-white focus:outline-none ring-1 ring-emerald-500/40 min-w-0"
          value={renameValue}
    ...
    {!isRenaming && (
      <button
        ...
        className="absolute right-1.5 top-1/2 -translate-y-1/2 w-6 h-6 flex items-center justify-center text-slate-500 hover:text-slate-300 rounded-lg hover:bg-white/[0.1] opacity-0 group-hover:opacity-100 transition-all"
      >
    ...
    {openMenuId === c._id && (
      <div
        onClick={(e) => e.stopPropagation()}
        className="absolute right-2 top-9 z-30 bg-[#1a2234] border border-white/[0.1] rounded-2xl shadow-xl shadow-black/40 w-36 overflow-hidden py-1.5"
      >
        <button onClick={onStartRename} className="w-full text-left px-3.5 py-2 text-sm text-slate-300 hover:bg-white/[0.06] flex items-center gap-2.5">
        ...
        <button onClick={onDelete} className="w-full text-left px-3.5 py-2 text-sm text-red-400 hover:bg-red-500/10 flex items-center gap-2.5">
          <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2}
