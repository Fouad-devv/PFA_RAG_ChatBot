...
const MD_COMPONENTS = (rtl) => ({
  ...
  ul:         ({ children }) => <ul className={`list-disc list-outside space-y-1 my-2 ${rtl ? 'mr-4' : 'ml-4'}`}>{children}</ul>,
  ol:         ({ children }) => <ol className={`list-decimal list-outside space-y-1 my-2 ${rtl ? 'mr-4' : 'ml-4'}`}>{children}</ol>,
